...
def main():
    mon = MongoDBCollection(mongo_username, 
                                ...
                                database_name,
                                collection_name)
    st.set_page_config(page_title='JobsForYou', page_icon=':sunglasses:', layout='wide', initial_sidebar_state='auto')
    ...
    # Display the website name with the defined style
    st.markdown(f'<div class="text">JobsForYou</div>', unsafe_allow_html=True)
    # Display the tagline with the defined style
    st.markdown(f'<div class="tagline">"Navigate Your Way to the Best Job with AI-Powered Resume Matching"</div>', unsafe_allow_html=True)
    st.markdown(text_style, unsafe_allow_html=True)
    instructions_placeholder = st.empty()
    instructions_placeholder.subheader(f'''How to use JobsForYou:
    1. Upload your resume in PDF format.
    2. Submit your resume by clicking on the “Submit” button.
    3. View the recommended jobs on the screen.''')
